...
module neural
  ...
  implicit none
  INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(15, 307) ! 至少15位有效数字, 指数范围307
...
contains
  ...
  subroutine neural_save_network(net, file)
    ...
    !open(newunit=file_num, file=file, status="replace")!status="replace" or status="new"
    open(newunit=file_num, file=file, status="replace", iostat=ierr) ! 正确
    if (ierr /= 0) stop "Failed to open file"
